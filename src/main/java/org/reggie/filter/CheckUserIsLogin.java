...
public class CheckUserIsLogin implements Filter {
  ...
  public void doFilter(
    ...
    if (!isIncludeInWhiteList(request.getRequestURI())
        && request.getSession().getAttribute(Session_Key) == null) {
      log.info(
          "用户未登录{}，{}", request.getRequestURI(), request.getSession().getAttribute(Session_Key));
      response.getWriter().write(JSON.toJSONString(Res.error("NOTLOGIN")));
    } else {
      log.info(
          "用户已登录{}，{}", request.getRequestURI(), request.getSession().getAttribute(Session_Key));

      Long empId = (Long) request.getSession().getAttribute(Session_Key);
